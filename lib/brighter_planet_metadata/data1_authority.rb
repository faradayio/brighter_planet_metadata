...
module BrighterPlanet
  class Metadata
    class Data1Authority
      ...
      def resources
        ::Dir[::File.expand_path(::File.join(::File.dirname($LOAD_PATH.grep(%r{gems/earth})[0]), 'lib', 'earth', '**', '*.rb'))].select { |f| ::File.read(f) =~ /class [\:A-Za-z0-9]+ < ActiveRecord::Base/ }.map { |path| ::File.basename(path, '.rb').camelcase }.sort
      end
      def datasets
